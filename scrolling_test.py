...
class MyWidget(Widget):
  ...
  def get_coordinates(self):
    if self.hero.map_position[0] < self.viewcols/2:
      x = 0
    elif self.hero.map_position[0] >= self.mymap.cols - (self.viewcols/2):
      x = self.mymap.cols - self.viewcols
    else:
      x = self.hero.map_position[0] - self.viewcols/2
    if self.hero.map_position[1] < self.viewrows/2:
      y = 0
    elif self.hero.map_position[1] >= self.mymap.rows - (self.viewrows/2):
      y = self.mymap.rows - self.viewrows
    else:
