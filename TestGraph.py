...
class Character(Image):
  ...
  def setChar(self,char):
    ...
    #else:
    #  self.source = 'atlas://roguelike16x16_gs_ro/' + self.char
  ...
  def __init__(self):
    ...
    for col in range(cols):
      ...
      for row in range(rows):
        ...
        self.Chr_matrix[col].append(Character(pos=position,size=size))
        self.Chr_matrix[col][-1].setChar(chr(ordn))
        with self.canvas:
          Color(*mycolor)
          Rectangle(pos=position,size=size,texture=self.Chr_matrix[col][-1].texture)
        ordn +=1
